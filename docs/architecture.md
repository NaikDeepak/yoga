...
| `src/lib/dates.ts` | IST date strings + due-date display format | `getISTDateString`, `formatDueDate` |
...
| `src/data/exercises.ts` | bilingual exercise library + per-patient prescriptions (`prescribed_exercises` join table; dose overrides fall back to library defaults; save replaces the whole set) | `listAllExercises`, `getPrescribedExercises`, `savePrescribedExercises` |
| `src/db/seed-exercises.ts` | idempotent exercise-library seeding from `src/db/seed-data/*.json` (used by tests and `scripts/seed-db.ts`) | `seedExercises` |
